...
const aiEditProfileName = ref(settingsStore.aiSettings.profiles.find((profile) => profile.id === settingsStore.aiSettings.activeProfileId)?.name || "");
...
const aiSelectedProfile = computed(() => aiProfileOptions.value.find((profile) => profile.id === aiEditProfileId.value) || aiProfileOptions.value[0]);
...
function savedCodexEnvSignature(): string {
  return JSON.stringify(Object.entries(normalizeAiEnv(aiSelectedProfile.value?.config.codexCliEnv)).sort(([left], [right]) => left.localeCompare(right)));
}
...
function currentAiEditProfile(): AiProfile {
  const name = aiEditProfileName.value.trim() || aiSelectedProfile.value?.name || AI_PROVIDER_PRESETS[aiEditProvider.value].label;
  return {
    id: aiEditProfileId.value || aiSelectedProfile.value?.id || uuid(),
    name,
...
function syncAiEditStateFromProfile(profile: AiProfile) {
  ...
  const provider = isWeb && config.provider === "codex-cli" ? "claude" : config.provider;
  ...
  aiEditAuthMethod.value = config.authMethod || AI_PROVIDER_PRESETS[provider].authMethod;
  aiEditEndpoint.value = provider === config.provider ? config.endpoint : AI_PROVIDER_PRESETS[provider].endpoint;
  aiEditModel.value = provider === config.provider ? config.model : AI_PROVIDER_PRESETS[provider].model;
  aiEditApiStyle.value = provider === config.provider ? config.apiStyle || "completions" : AI_PROVIDER_PRESETS[provider].apiStyle;
...
function syncAiEditState() {
  syncAiEditStateFromProfile(aiSelectedProfile.value || settingsStore.aiSettings.profiles[0]);
}
...
function aiHasChanges(): boolean {
  ...
  return (
    ...
    aiEditApiKey.value !== config.apiKey ||
    aiEditAuthMethod.value !== (config.authMethod || AI_PROVIDER_PRESETS[config.provider].authMethod) ||
...
function aiDeleteProfile() {
  if (aiProfileOptions.value.length <= 1) return;
  settingsStore.deleteAiProfile(aiEditProfileId.value);
  syncAiEditState();
}
          ...
          <div class="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-1 pr-2">
                ...
                <div class="grid grid-cols-3 items-center gap-3">
                  ...
                  <div class="col-span-2 flex min-w-0 items-center gap-2">
                    <Select :model-value="aiEditProfileId" @update:model-value="(v: any) => aiSelectProfile(String(v))">
                      <SelectTrigger class="min-w-0 flex-1" inputClass="h-8 text-xs">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <Button type="button" size="icon" variant="outline" class="h-8 w-8 shrink-0" :title="t('ai.addProfile')" :aria-label="t('ai.addProfile')" @click="aiAddProfile">
                      <Plus class="h-3.5 w-3.5" />
                    </Button>
                    <Button type="button" size="icon" variant="outline" class="h-8 w-8 shrink-0" :disabled="aiProfileOptions.length <= 1" :title="t('ai.deleteProfile')" :aria-label="t('ai.deleteProfile')" @click="aiDeleteProfile">
            ...
            <section v-else-if="activeSettingsTab === 'about'" class="flex flex-col gap-5 py-2">
              ...
              <div class="grid gap-3 sm:grid-cols-2">
                ...
                <button type="button" class="rounded-lg border p-4 text-left transition-colors hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" @click="openExternalUrl('https://github.com/yes01/dbx')">
                  <div class="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                    {{ t("settings.project") }}
                  </div>
                  <div class="mt-3 flex items-center gap-2 text-sm font-medium">
                  ...
                  <div class="mt-1 text-sm text-primary">github.com/yes01/dbx</div>
                </button>
