...
import { buildCreateExtensionSql, buildDropExtensionSql } from "@/lib/dbAdminSql";
...
async function loadData() {
  ...
  loading.value = true;
  try {
    const schema = props.node.schema || "public";
    const [avail, inst] = await Promise.all([api.listAvailableExtensions(props.node.connectionId, props.node.database).catch(() => [] as ExtensionInfo[]), api.listExtensions(props.node.connectionId, props.node.database, schema).catch(() => [] as ExtensionInfo[])]);
