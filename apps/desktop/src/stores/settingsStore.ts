...
export function normalizeAiSettings(value: Partial<AiSettings> | Partial<AiConfig> | null | undefined): AiSettings {
  if (isAiSettingsValue(value)) {
    ...
    if (profiles.length) {
      const activeProfileId = profiles.some((profile) => profile.id === value.activeProfileId) ? value.activeProfileId || profiles[0].id : profiles[0].id;
...
export const useSettingsStore = defineStore("settings", () => {
  ...
  function setAiSettings(settings: AiSettings) {
    ...
    aiConfig.value = aiSettings.value.profiles.find((profile) => profile.id === aiSettings.value.activeProfileId)!.config;
  }
  ...
  function updateAiConfig(config: Partial<AiConfig>, profileId = aiSettings.value.activeProfileId) {
    ...
    if (profile.id === aiSettings.value.activeProfileId) {
